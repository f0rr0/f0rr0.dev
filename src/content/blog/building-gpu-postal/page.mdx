...
<figure>
  <Image
    src="./model-pipeline.webp"
    alt="A paper-cut diagram showing address text becoming UTF-8 byte features, passing through local context and forward and backward affine scans, then BIO scores and CRF decoding before returning original UTF-16 text spans."
  />
  <figcaption>
    The model keeps the original characters at the edges of the system: bytes go
    in, and spans over the user's text come back out.
  </figcaption>
</figure>

...
![Cut-paper portraits of Sid in several head poses, with the central face turned toward a saffron mouse cursor.](./opengraph-image.jpg)
...
![An early four-by-four contact sheet with sixteen individual portraits labeled by row, column, intended direction and turn strength.](./contact-sheet-poses.png)
...
![The original 57-frame review contact sheet, labeled F00 through F56 with each endpoint direction or intermediate transition and percentage.](./contact-sheet-frames.png)
...
![The original motion reviewer with a labeled portrait, movement and speed selectors, frame stepping, a scrubber, and a Flag this frame and pause button above the feedback box.](./motion-review.png)
...
![The original comparison tool overlays two bottom-left portraits at 50% opacity, with eye-line guides and a lower-torso measurement region. The readout reports 0.4% changed pixels in that region.](./portrait-overlay.png)
...
The cursor controller uses `atan2` to choose one of eight directions, with a neutral dead zone around the portrait. Neighboring poses have animated transitions; other turns switch directly to the requested pose. That small graph avoids generating transitions between every possible pair of poses.

Retargeting matters more than it first appears. Reversing halfway through a turn can make the portrait jitter when the cursor hovers near a direction boundary. The controller ignores pointer changes until the current turn finishes, then responds to fresh movement. The same three intermediate images work in both directions.
...
The header advances frames every 50ms while turning, then stops. It decodes the atlas before displaying it, respects reduced motion, and uses slower, occasional glances on phones.
